...
class RBFAutoRegressiveGaussianObservation:

    def __init__(self, nb_states, dm_obs, mu_basis, sigma_basis, prior, reg=1e-32):
        self.nb_states = nb_states
        self.dm_obs = dm_obs
        self.mu_basis = mu_basis
        self.sigma_basis = sigma_basis
        self.n_basis = len(mu_basis)
        ...
        self.W = np.zeros((self.nb_states, self.dm_obs, self.n_basis))
    ...
    @property
    def params(self):
        return self.W, self._sqrt_cov

    @params.setter
    def params(self, value):
        self.W, self._sqrt_cov = value
    ...
    def initialize(self, x, u, **kwargs):
        ...
        for k in range(self.nb_states):
            ...
            # TODO : write function to estimate weights
    ...
    def log_likelihood(self, x, u): ## TO CHECK
        loglik = []
        for _x, _u in zip(x, u):
            _loglik = np.column_stack([lg_mvn(_x[1:, :], self.mean(k, _x[:-1, :], _u[:-1, :self.dm_act]), self.cov[k])
                                       for k in range(self.nb_states)])
    ...
    def mstep(self, gamma, x, u, weights=None, use_prior=False): ## TO CHECK
        aux = []
        if weights:
            for _w, _gamma in zip(weights, gamma):
               aux.append(_w[:, None] * _gamma)
    ...
    def smooth(self, gamma, x, u): ## TO CHECK
        mean = []
        for _x, _u, _gamma in zip(x, u, gamma):
            _mu = np.zeros((len(_x) - 1, self.nb_states, self.dm_obs))
            for k in range(self.nb_states):
